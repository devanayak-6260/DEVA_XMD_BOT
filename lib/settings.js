...
function save() {
  try {
    const keys = [
      ...
      'autoTyping', 'autoRecording', 'autoCallReject', 'antiDelete', 'customReactEmoji'
    ];
    const data = {};
    for (const key of keys) data[key] = key === 'customReactEmoji' ? (Array.isArray(config[key]) ? config[key] : []) : Boolean(config[key]);
